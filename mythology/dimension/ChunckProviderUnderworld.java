...
public class ChunckProviderUnderworld implements IChunkProvider {
	...
	public void func_147424_a(int var1, int var2, Block[] block) {
		...
		this.biomesForGeneration = this.worldObj.getWorldChunkManager()
				.getBiomesForGeneration(this.biomesForGeneration, var1 * 4 - 2,
						var2 * 4 - 2, 10, 10);
	...
	public void replaceBlocksForBiome(int i, int j, Block[] block, byte[] b,
			BiomeGenBase[] biomegenbase) {
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(
				this, i, j, block, biomegenbase);
		...
		this.stoneNoise = this.field_147430_m.func_151599_a(this.stoneNoise,
				(double) (i * 16), (double) (j * 16), 16, 16, d0 * 2.0D,
				d0 * 2.0D, 1.0D);

		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				BiomeGenBase biomegenbased = biomegenbase[l + k * 16];
				biomegenbased.genTerrainBlocks(this.worldObj, this.rand, block,
						b, i * 16 + k, j * 16 + l, this.stoneNoise[l + k * 16]);
			}
		}
	}
	...
	public void populate(IChunkProvider IChunkProvider, int par2, int par3) {
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(
				IChunkProvider, worldObj, rand, par2, par3, flag));
		...
		if (biomegenbase != BiomeGenBase.desert
				...
				&& TerrainGen.populate(IChunkProvider, worldObj, rand, par2,
						par3, flag, LAKE)) {
		...
		if (TerrainGen.populate(IChunkProvider, worldObj, rand, par2, par3,
		...
		boolean doGen = TerrainGen.populate(IChunkProvider, worldObj, rand,
		...
		doGen = TerrainGen.populate(IChunkProvider, worldObj, rand, par2, par3,
				flag, ICE);
	 ...
	 * given location.
	 */
	public List getPossibleCreatures(EnumCreatureType EnumCreatureType, int x,
			int y, int z) {
		BiomeGenBase biomegenbase = this.worldObj.getBiomeGenForCoords(x, z);
		return EnumCreatureType == EnumCreatureType.monster
				&& this.scatteredFeatureGenerator.func_143030_a(x, y, z) ? this.scatteredFeatureGenerator
				.getScatteredFeatureSpawnList() : biomegenbase
				.getSpawnableList(EnumCreatureType);
	}

	public ChunkPosition func_147416_a(World world, String var1, int x, int y,
			int z) {
		return "Stronghold".equals(var1) && this.strongholdGenerator != null ? this.strongholdGenerator
				.func_151545_a(world, x, y, z) : null;
	}
